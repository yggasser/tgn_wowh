...
function featureToCardObject(feature){
  const p=((feature && feature.properties) && typeof feature.properties==="object") ? feature.properties : {};
  return {
    id:p.id,
...
async function openPopupFor(id,latlng){
  ensurePopupStyles();
  const popup=L.popup({className:"tg-popup",maxWidth:660,minWidth:320,autoPan:true})
    .setLatLng(latlng)
  ...
  const showError=(msg)=>{
    ...
    popup.setContent(`<div class="tg-card">
      ...
      <div class="tg-meta">Адрес: ${escapeHtml(idToAddress(id))}</div>
      <div class="tg-warning" style="border-color:#ffd0d0;background:#fff3f3;color:#7a0000">Не удалось загрузить карточку: ${escapeHtml(String(msg||"unknown error"))}</div>
      <div class="tg-meta">URL: /api/object/${escapeHtml(encodeURIComponent(id))}</div>
    </div>`);
    protectPopupInteractions(popup);
  };
